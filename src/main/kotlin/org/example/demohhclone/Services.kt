...
class VacancyServiceImpl(
    ...
    override fun searchVacancies(searchText: String, pageable: Pageable): Page<VacancyResponseDto> {
        return vacancyRepository.findAllNotDeleted(pageable).map { vacancyMapper.toDto(it) }
    }
...
class UpdateDatabase(
    ...
    fun updateDatabase(){
        ...
        requestObjects.forEach { requestObject ->
            fetchAllVacancies(requestObject.name)
        }
    ...
    fun scheduledFetchAllVacancies(name: String) {
        fetchAllVacancies(name, 20)
    }

    fun fetchAllVacancies(searchText: String , perPage: Int = 20) {

        val firstPage = hhApiClient.getVacancies(searchText, perPage, 0)
        val totalPages = firstPage.pages


        processVacancies(firstPage.items)

        for (page in 1 until totalPages) {

            val response = hhApiClient.getVacancies(searchText, perPage, page)
            processVacancies(response.items)
    ...
    private fun processVacancies(vacancies: List<HHResponseDto>) {
        vacancies.forEach { vacancy ->
            entitySaveService.saveVacancy(vacancy)
        }
    }
}
...
class EntitySaveService(
    ...
    fun saveVacancy(dto: HHResponseDto): Vacancy {
        ...
        val employmentType = saveOrGetEmploymentType(dto.employmentType)
    ...
    private fun updateVacancyProfessionalRoles(
        ...
        val rolesToAdd = dto.professionalRoles?.filter { it.id !in existingRoleIds }
